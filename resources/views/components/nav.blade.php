<nav class="pb-6 px-4">
    <div class="flex flex-wrap justify-center gap-x-6 gap-y-3 md:gap-x-10 font-serif text-base">

        <a href="/photos" class="text-stone-500 hover:text-black transition-colors duration-300">
            Photos
        </a>

        <a href="/#venue" class="text-stone-500 hover:text-black transition-colors duration-300">
            Venue
        </a>

        <a href="/#faq" class="text-stone-500 hover:text-black transition-colors duration-300">
            FAQ
        </a>

        <a href="/#sponsors" class="text-stone-500 hover:text-black transition-colors duration-300">
            Sponsors
        </a>

        <a href="/#registration" class="text-stone-500 hover:text-black transition-colors duration-300">
            Registration
        </a>

    </div>
</nav>